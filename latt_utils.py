...
def f_Latt_Angle(p1,p2,p3,latt=None):
  r12 = f_Latt_Distance(p1,p2,latt) 
  r13 = f_Latt_Distance(p1,p3,latt)
  r23 = f_Latt_Distance(p2,p3,latt)

  arg = (r12**2 + r13**2 - r23**2) / (2.0*r12*r13)
  arg = max( min(arg,1.0), -1.0)    # to avoid numerical noises 
...
def f_Intern_to_Cart(xyz,latt,iop=0):
  """ 
...
def f_Latt_Struct_I2C(bas,latt,iop=0):
  """ 
  ...
  for iat in range(nat):
    atom = bas[iat][0]
    xyz = bas[iat][1][:]
    xyz_c = f_Intern_to_Cart(xyz,latt,iop)
    bas_c.append( [atom, xyz_c] ) 
  return bas_c

def f_Latt_Vec2Const(vec):
  """
