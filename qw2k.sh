...
Usage: `basename $0` [options]  <job_name [job_options]>
  Options:
   ...
   -m <mpi=1>        # the number of precesses used for MPI parallelization. if mpi < 0, lapw0mpi is not used 
   ...
   -v <w2kver=14.2>    # wien2k version (073/091/11/12/13/14/14.2)
...
Please be sure that the number of processes used in k-mesh parallelization should not be larger than the number of k-points 
(the actual number of k-points in case.klist, not the total number of k-points in the BZ)
...
#WIENDIR=${WIENDIR:=$HOME/prog/wien2k}
interactive=0
emptyrun=0
w2kver=14.2
...
export WIENROOT=$WIENDIR/v$w2kver
...
echo " exec_basename=" $exec_basename
echo " exec_fullname=" $exec_fullname 
...
#PBS -M $MAIL
  ...
  setenv WIENROOT $WIENDIR/v$w2kver
  cd $workdir 
...
#    setenv SCRATCH ./scr-$jobname 
  else 
    setenv SCRATCH .
#    setenv SCRATCH $scrdir/scr-$jobname
  endif 
...
then
  ...
  echo ''
  if [ $interactive -eq 1 ]; then
    echo " run the job interactively"
    $subsh 
  else
    echo " run the job locally"
    nohup $subsh >& q-$output &
  fi
else
  qsub -q $queue -N $jobname $subsh
  echo "a job has been submitted to the queue " 
fi
